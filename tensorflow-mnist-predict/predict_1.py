...
def predictint(imvalue):
    ...
    with tf.Session() as sess:
        ...
        #a=prediction.eval(feed_dict={x: [imvalue]})
        #print a;
        prediction=tf.argmax(y,1)   
        print "predictions", prediction.eval(feed_dict={x: [imvalue]}, session=sess)
        possiblity =sess.run(y, feed_dict={x: [imvalue]})
        a=list(possiblity)
        #print a[0]
        i=0
        for i in range(0,10):
            print "The %d possiblity:%.4f\t\n"%(i,a[0][i])
        #b=sorted(a[0])
       # print b
